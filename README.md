...
I'm working on a few ThinkPad keyboards. I'm using X1 as an example in this doc.

* [Keyboard 1: X1](x1.md)
* [Keyboard 2: X390, X395, and X280](x390.md)
* [Keyboard 3: T14](t14.md)

## Reverse Engineering
The following documents how I reverse engineered the pinout of the TrackPoints and the matrices of the keyboards.

A typical ThinkPad TrackPoint is a PS/2 device (so are most touchpads). It has 8 pins: Vcc, ground, data, clock, reset, and 3 for buttons. The buttons are usually external to the TrackPoint unit and integrated with the keys instead. When the buttons are pressed, the pins are connected to ground.

#### TrackPoint
Many ThinkPad TrackPoints use TPM754 chip. The IC on this one is unmarked but it has the same number of pins and layout. So I suspect it's also a TPM754.
...
There's no reason why Lenovo would deviate from this implementation, since the datasheet was provided by Philips and the chip runs IBM's code. Sure enough, all of these pins are connected to pins on the inerface. I don't even need to do any guess work!
...
I used an Arduino Mega 2560 to determine the matrix, because it has a lot of IO pins. It is the same approach I used in [this project](https://github.com/delingren/hp_x360_keyboard).

## Firmware
I've always used QMK for keyboard projects. It's the most popular firmware in the mechanical keyboard community. It also has support for PS/2 pointing devices. So that's what I'm using. Another option is ZMK, which is the new kid on the block. I don't have experiences with it. PS/2 support in QMK is new to me, so I played with it a little bit with a few dev boards in my drawer:

* Teensy & ProMicro (ATMega32U4)
* Pi Pico (RP2040)
* STM32G474

On ATMega32U4, there are three options for PS/2: busy wait (only works in remote mode, not stream mode), interrupt, and usart. Busy wait is not really an option. It's a polling method. The other two are both fine. But usart needs to use D5 pin, which is not available on ProMicro, unless you remove an onboard LED. On STM32 chips, I never got usart to work. But interrupt works fine.

I did some prototyping on all the dev boards following the instructions from [QMK docs](https://docs.qmk.fm/features/ps2_mouse). With Teensy, I set clock pin to D5 and data pin to D2 and it worked with both usart and interrupt drivers. With ProMicro, I set clock pin to D2 and data pin to D3 and it worked with the interrupt driver.
...
For STM32, also add this to `halconf.h`:
...
* The backlight uses another one, if we want to utilize it.
...
* Try to compress the matrix a little. The matrix is rather sparse. `Fn` can be easily foled into another row and another column. The `Alt` and `Ctrl` buttons can also be folded into one row. Ghosting is not really a concern here. I can reduce 26 pins to 22 this way.
...
While researching on F405, I also stumbled upon WeAct STM32G474 and STM32G431, which seem to be good candidates too. They lack the TF card reader, compared with F405, which I prefer since I don't need it anyway.
