...
The trackpoint is not complicated either. The button pins needed to be properly wired; data and clock pins are connected to GPIO pins (more in the next section); The trackpoints seem to be able to handle both 3.3 volts and 5 volts. So Vcc should match the MCU. Since I'm going to use RP2040, which is not 5v tolerant, I'm powering the trackpoint with 3.3v.
...
## Firmware
I've always used QMK for keyboard projects. It's the most popular firmware in the mechanical keyboard community. It also has support for PS/2 pointing devices. So that's what I'm using. Another option is ZMK, which is the new kid on the block. I don't have experiences with it.

For the MCU, I have a few options, based on what I have in my drawer.
...
The fromer two are based on ATMega32U4. The latter two are based on RP2040. I also want to use a USB-C interface. Some of my ProMicros have USB-C ports. Teensy 2.0 has a mini USB port. So I'd like to avoid it if possible. Otherwise I'd need to use a USB-C breakout board for the external interface.

The keyboard functionality works smoothly on both. But the PS/2 mouse implementation varies. On ATMega32U4, there are three options: busy wait (only works in remote mode, not stream mode), interrupt, and usart. Busy wait is not really an option. It's a polling method. The other two are both fine. But usart needs to use D5 pin, which is not available on ProMicro, unless you remove an onboard LED.
